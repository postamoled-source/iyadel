            ...
            <div className="grid grid-cols-4 gap-2">
              {CATEGORY_CARDS.map(({ label, cat, Icon }) => (
                <button key={label} onClick={() => { setActiveCategory(cat); trackEvent("category_select", { category: cat }); }}
                  className={`flex flex-row items-center h-12 px-2.5 rounded-xl bg-white dark:bg-[#2D2A5A] border transition-all duration-300 hover:scale-[1.02] ${activeCategory === cat ? "border-[#6D28D9] shadow-[0_4px_12px_rgba(109,40,217,0.18)]" : "border-[#F3F4F6] dark:border-[#4B3F8A] shadow-[0_2px_6px_rgba(109,40,217,0.08)]"}`}>
                  <div className="w-7 h-7 min-w-[28px] rounded-lg bg-gradient-to-br from-[#6D28D9] to-[#F59E0B] flex items-center justify-center shadow-[0_4px_10px_rgba(109,40,217,0.25)]">
                    <Icon className="w-4 h-4 text-white" strokeWidth={2.2} />
                  </div>
                  <span className="ml-2 text-[11px] font-bold text-[#111827] dark:text-[#FEF3C7] truncate">{t(label)}</span>
                </button>
              ))}
            </div>
          </div>
